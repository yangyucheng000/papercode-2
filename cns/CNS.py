...
equation = Equation.sixhump
#'sixhump','branin','griewank','sphere','fir','engineering'

if equation== Equation.sixhump:        optimal = -1.03160000000000
...
while iii<1:
    ...
    for i in range(0,1):
        for j in range(0,N):
            ...
            #%%%%%%%%%%牛顿法更新位移 %%%%%%%%%
            #求逆
            xcns[:, j] = x[:, j] - (hessianRes.inverse()) @ (jacobiRes + beta * error_sum[:, j])
            if equation==Equation.engineering:
                xcns[0, j] = round(xcns[0, j] / 0.0625) * 0.0625
                xcns[1, j] = round(xcns[1, j] / 0.0625) * 0.0625
                if xcns[0, j]<0.6 or xcns[0, j] > 2:
                    xcns[0, j] = round((np.rand() * (2 - 0.6) + 0.6) / 0.0625) * 0.0625
                if xcns[1, j] < 0.6 or xcns[1, j] > 2:
                    xcns[1, j] = round((np.rand() * (2 - 0.6) + 0.6) / 0.0625) * 0.0625
                if xcns[2, j] < 10:
                    xcns[2, j] = 10
                if xcns[3, j] < 50 or xcns[3, j] > 240:
                    xcns[3, j] = np.rand() * (240 - 50) + 50
            if equation==Equation.engineering:
                if cons1(xcns[:,j]) == 1:
                    if cons2(xcns[:,j]) == 1:
                        if cons3(xcns[:,j]) == 1:
...
#            %%%%%%%%%%更新全局最优位置和最优值 %%%%%%%%%
            ...
            #%%%%%%%%%%更新位置和速度值 %%%%%%%%%
            s=time.time()
            x[:, j] = x[:, j] + w * (xcns[:, j] - x[:, j]) + c1 * np.rand() * (p[:, j] - x[:, j]) + c2 * np.rand() * (g - x[:, j])
            e=time.time()
            for ii in range(D):
                if (x[ii, j] > Xmax) or (x[ii, j] < Xmin):
                    x[ii, j] = np.rand().view(()) * (Xmax - Xmin) + Xmin
        gb[0][i] = gbest
        print(gbest)
        #gbest_index[:, i] = g
    ...
    for i in range(1,T):
        ...
        for j in range(N):
            ...
            x_=x[:,j]
            xc=xcns[:, j]
            x[:, j] = x_ + w * (xc - x_) + c1 * np.rand().view(())* (p[:, j] - x_) + c2 * np.rand().view(()) * (g - x_)
            #e=time.time()
            #print("{:.2f}".format(e - s))
            if equation== Equation.engineering:
                ...
                if x[0, j] < 0.6 or x[0, j] > 2:
                    ...
                    x[0, j] = (np.rand().view(()) * (2 - 0.6) + 0.6)

                if x[1,j] < 0.6 or x[1,j] > 2:
                    #% x[1,j] = round((rand * (2 - 0.6) + 0.6) / 0.0625) * 0.0625;
                    x[1,j] = (np.rand().view(()) * (2 - 0.6) + 0.6)
                if (x[2,j] < 10):
                    x[2,j] = 10
                if (x[3,j] < 50 or x[3,j] > 240):
                    x[3,j] = np.rand().view(())* (240 - 50) + 50
            else:
                for ii in range(0,D):
                    if (v[ii,j] > Vmax) or (v[ii,j] < Vmin):
                        v[ii,j] = np.rand().view(()) * (Vmax - Vmin) + Vmin
                    if (x[ii,j] > Xmax) or (x[ii,j] < Xmin):
                        x[ii,j] = np.rand() .view(())* (Xmax - Xmin) + Xmin
    ...
    if (isplot == 1):
        ...
        plt.xlabel('迭代次数')
        plt.ylabel('适应度值')
        plt.ylim(gb[0].numpy().min(),gb[0].numpy().max())
        plt.title('适应度进化曲线')
        plt.plot(gb[0].numpy())
        plt.show()
        print(gb[0].numpy())
...
print(['CNS标准差：', 0.])
print(['CNS均值：' ,save_value['1']])
print(['beta,tesnum：',beta,tesnum])
