...
import datetime
import time
from src.loss import NetWithLossV2, NetWithLossFptV2, MyTrainOneStepCellV2, box_ious, MyTrainOneStepCell, MyTrainOneStepCellAccumulator, MyTrainOneStepNoScaleAccumulator
import logging
import warnings
...
def init_lr(step_size, whos_lr):
    ...
    if args.lr_scheduler == 'step':
        lr_list = generate_steps_lr(0.01 * base_lr, base_lr, step_size, args.max_epochs, args.warmup_steps, args.drop_epochs)
        return lr_list
...
def train_loop(net, train_dataloader, epoch_idx, logFile, local_rank):
    net.set_train(True)
...
if __name__ == '__main__':
    ...
    # lr_list = init_lr(step_size, args.lr_base)
    # lr_base = init_lr(step_size, args.lr_base)
    # lr_visual = init_lr(step_size, args.lr_visual)
    # lr_lang = init_lr(step_size, args.lr_lang)
    ...
    if args.use_selector:
        params_groups.append({'params': [p for n, p in net.parameters_and_names() if n.startswith('network.selector') and p.requires_grad], 'lr': args.lr_base, 'weight_decay': 0.0})

    # define optimizer
    optimizer = nn.AdamWeightDecay(params=params_groups, learning_rate=args.lr_base, weight_decay=args.weight_decay)
    ...
    for epoch_idx in range(args.max_epochs):
        train_loop(net, data_loader, epoch_idx, logFile, local_rank)
        # eval
