...
def shoot_infs(inp_tensor):
    mask_inf = ops.isinf(inp_tensor)
    ind_inf = ops.nonzero(mask_inf)
    if len(ind_inf) > 0:
        for ind in ind_inf:
            if len(ind) == 2:
...
class MAEForPretrain(nn.Cell):
    ...
    def forward_features_test(self, x, mask):
        x = self.patch_embed(x)
        bsz = x.shape[0]
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        loss = loss.mean(axis=-1) #N,L
        mask = mask.astype(loss.dtype)#N,L
        loss = (loss * mask).sum() / mask.sum()

        
        l2_normalize = ops.L2Normalize(axis = 1)
        # softmax = nn.Softmax(axis=1)
        m = ops.softmax(l2_normalize(target.mean(axis=-1)) / 0.07, axis = 1)#N,L
        n = ops.softmax(l2_normalize(pred.mean(axis=-1)) / 0.1, axis = 1)#N,L
        log = ops.Log()

        logn = shoot_infs(log(n))
        loss_aug = - (m * logn).sum(axis=1).mean()

        # prob_sum = n.sum(axis=0)
        # prob_sum = prob_sum / m.shape[0]
        
        # prob_sum = -(math.log(prob_sum.shape[-1]) + log(prob_sum).mean())

        if self.iter_epoch < 120:
            scale = 0.0
        elif self.iter_epoch < 150:
            scale = (self.iter_epoch - 150) / float(30)
        else:
            scale = 1.0

        loss = scale * (loss_aug) + (1 - scale) * loss
