...
def to_tensor(adj, features, labels=None, device='cpu'):
    ...
    if sp.issparse(adj):
        ...
        adj = mindspore.Tensor(adj)
    # if sp.issparse(features):
    #     features = sparse_mx_to_torch_sparse_tensor(features)
    # else:
    #     features = mindspore.Tensor(np.array(features))
...
def KDE(A, Z):
    ...
    #print(A_Z.get_device())
    
    MI = MutualInformation(device=A_Z.get_device(),
        sigma=0.4, num_bins=A_Z.shape[0], normalize=True)
...
class MutualInformation(nn.Module):
    ...
    def __init__(self, device,sigma=0.4, num_bins=256, normalize=True):
