...
class MAEForPretrain(nn.Cell):
    ...
    def forward_features_test(self, x):
        x = self.patch_embed(x)
        bsz = x.shape[0]
        ...
        attn = attn_list[-1]
        cls_attn_weight = attn[:, :, 1:, 0].mean(-1).reshape(bsz, -1, 1)
        cls_attn = attn[:, :, 0, 1:].mul(cls_attn_weight).mean(1)
        img_attn_weight = attn[:, :, 0, 0].reshape(bsz, -1, 1)
        img_attn = attn[:, :, 1:, 1:].mean(2).mul(img_attn_weight).mean(1)
        attn_map = cls_attn.add(img_attn)
        attn_map = attn_map.reshape(bsz, -1)#N, 196
        mask_idx = ops.argsort(attn_map, axis=1, descending=True)
        mask_idx = mask_idx[:, :int(np.ceil(196 * 0.75))]
        mask = ops.zeros((bsz, 196), dtype=ms.int32)
        for i in range(bsz):
            mask[i][mask_idx[i]] = 1
        return mask
    ...
    def construct(self, imgs, mask):
        ...
        if self.iter_epoch <= 40:
            mask = ops.reshape(mask, (bsz, -1))
        else:
            mask = self.forward_features_test(imgs)
